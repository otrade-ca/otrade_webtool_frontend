...
export const addInfluence = (influence, stakeholderId) => async (
    ...
    try {
        ...
        const {
            data: { data }
        } = await axios.post(`/api/v1/stakeholders/${stakeholderId}/influences`, influence, config);
...
// get influence
export const getInfluenceDetails = (id) => async (
    dispatch, getState
) => {
    try {
        ...
        const {
            data: { data },
        } = await axios.get(`/api/v1/influences/${id}`, config);

        dispatch({type: INFLUENCE_DETAILS_SUCCESS, payload: data});
    } catch (error) {
        dispatch({type: INFLUENCE_DETAILS_FAIL,
        payload:
				error.response && error.response.data.message
					? error.response.data.message
					: error.message,
...
// delete influence
export const deleteInfluence = (id) => async (
    dispatch, getState
) => {
    try {
        ...
        dispatch({type: INFLUENCE_DELETE_FAIL,
        payload:
				error.response && error.response.data.message
					? error.response.data.message
					: error.message,
...
export const listInfluences = (stakeholderId) => async (
    dispatch, getState
) => {
    try {
        ...
        const {
            ...
            `/api/v1/stakeholders/${stakeholderId}/influences`, 
            config
        );

        dispatch({type: INFLUENCE_LIST_SUCCESS, payload: data});
    } catch (error) {
        dispatch({type: INFLUENCE_LIST_FAIL,
        payload:
				error.response && error.response.data.message
					? error.response.data.message
					: error.message,
